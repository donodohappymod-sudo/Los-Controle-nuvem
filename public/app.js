...
async function api(path,opt={}){const r=await fetch(path,{headers:{'content-type':'application/json',...(opt.headers||{})},...opt});const j=await r.json().catch(()=>({error:'Resposta inválida'}));if(!r.ok)throw Error(j.error||'Erro');return j}
...
const nav=[['home','⌂','Início'],['sources','🌐','Fontes'],['library','▦','Biblioteca'],['diagnosis','🩺','Diagnóstico'],['monitoring','◉','Monitoramento'],['merge','🔀','Mesclar'],['generated','📡','Fontes Geradas'],['studio','✦','Studio'],['settings','⚙','Configurações']];

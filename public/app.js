(()=>{const root=document.querySelector('#app');const S={user:null,page:'home',data:{},busy:false};
...
const APP_VERSION='20260923-fix1';window.addEventListener('pageshow',e=>{if(e.persisted){console.log('[LOS] pageshow restaurado pelo Safari; mantendo estado da aplicação.')}});const nav=[['home','⌂','Início'],['sources','🌐','Fontes'],['library','▦','Biblioteca'],['diagnosis','🩺','Diagnóstico'],['monitoring','◉','Monitoramento'],['merge','🔀','Mesclar'],['generated','📡','Fontes Geradas'],['studio','✦','Studio'],['settings','⚙','Configurações'],['ai','🧠','AI / Testes']];
...
async function pageMerge(){const s=await api('/api/sources');return '<div class="toolbar"><div><h2>Mesclar fontes</h2><p class="muted">Selecione fontes, normalize e remova duplicados antes de gerar.</p></div><button class="primary" id="previewMerge">Analisar mesclagem</button></div><section class="panel"><div class="checks">'+s.items.map(x=>'<label><input type="checkbox" class="source-check" value="'+x.id+'"><span><b>'+esc(x.name)+'</b><small>'+x.channels+' canais · '+x.movies+' filmes · '+x.series+' séries · '+x.episodes+' episódios</small></span></label>').join('')+'</div></section><div id="mergeResult"></div>'}
async function pageGenerated(){
 const d=await api('/api/generated');
 const rows=d.items.map(x=>'<tr><td>'+esc(x.name)+'</td><td>'+x.itemCount+'</td><td>'+new Date(x.createdAt).toLocaleString("pt-BR")+'</td><td><a class="mini" href="/api/generated/'+x.id+'" target="_blank">Abrir M3U</a></td></tr>').join('');
 return '<div class="toolbar"><div><h2>Fontes geradas</h2><p class="muted">Playlists M3U produzidas pelo sistema e prontas para usar como URL de playlist.</p></div></div><div class="panel"><div class="table-wrap"><table><thead><tr><th>Nome</th><th>Itens</th><th>Data</th><th>Ações</th></tr></thead><tbody>'+rows+'</tbody></table></div></div>';
...
async function pageAI(){const d=await api('/api/ai/diagnostics');return '<div class="toolbar"><div><h2>🧠 AI / Testes</h2><p class="muted">Diagnóstico automático do servidor, banco, coleta e integrações de IA.</p></div><button class="primary" id="runAI">Executar novamente</button></div><section class="panel"><div class="stats">'+d.checks.map(x=>'<div class="stat"><span>'+(x.ok?'🟢':'🔴')+'</span><small>'+esc(x.name)+'</small><strong>'+(x.ok?'OK':'ERRO')+'</strong><p class="muted">'+esc(x.detail)+'</p></div>').join('')+'</div><div class="panel"><h3>Integrações</h3><p>GPT: <b>'+(d.integrations.gpt?'configurado':'não configurado')+'</b> · Codex: <b>'+(d.integrations.codex?'configurado':'não configurado')+'</b></p><p class="muted">As chaves ficam no servidor do Render, nunca no navegador.</p></div>'+(d.ai?'<div class="panel"><h3>Análise do GPT</h3><pre class="preview">'+esc(d.ai)+'</pre></div>':'<div class="panel"><p class="muted">O diagnóstico local funciona sem chave. Para análise pelo GPT, configure OPENAI_API_KEY no Render.</p></div>')+'</section>'}async function pageSettings(){return '<div class="toolbar"><div><h2>Configurações</h2><p class="muted">Segurança da conta.</p></div></div><section class="panel narrow"><h3>Alterar senha</h3><form id="pwForm" class="form"><label>Senha atual<input name="currentPassword" type="password" required></label><label>Nova senha<input name="newPassword" type="password" minlength="10" required></label><button class="primary">Atualizar senha</button></form></section>'}
async function render(){if(!S.user)return login();shell();const view=document.querySelector('#view');try{S.search=S.search||'';const pages={home:pageHome,sources:pageSources,collection:pageCollection,library:pageLibrary,diagnosis:pageDiagnosis,monitoring:pageMonitoring,merge:pageMerge,generated:pageGenerated,studio:pageStudio,settings:pageSettings,ai:pageAI};view.innerHTML=await pages[S.page]();bind()}catch(e){view.innerHTML='<div class="panel error">Erro: '+esc(e.message)+'</div>'}}
...
document.querySelectorAll('.copy').forEach(b=>b.onclick=()=>navigator.clipboard?.writeText(b.dataset.url).then(()=>alert('URL copiada')).catch(()=>alert(b.dataset.url)));
document.querySelectorAll('.delitem').forEach(b=>b.onclick=async()=>{if(confirm('Excluir item?')){await api('/api/items/'+b.dataset.id,{method:'DELETE'});render()}});
const rd=document.querySelector('#runDiag');if(rd)rd.onclick=async()=>{rd.disabled=true;rd.textContent='Verificando...';try{const x=await api('/api/diagnosis',{method:'POST'});document.querySelector('#diagResult').innerHTML='<h3>Resultado</h3>'+cards({sources:0,channels:0,movies:0,series:0,episodes:x.total,online:x.online,errors:x.errors})+'<p>'+x.total+' analisados · '+x.timeouts+' timeout(s).</p>'}catch(e){document.querySelector('#diagResult').innerHTML='<div class="error">'+esc(e.message)+'</div>'}rd.disabled=false;rd.textContent='Executar agora'};
const rm=document.querySelector('#runMon');if(rm)rm.onclick=async()=>{rm.disabled=true;try{const x=await api('/api/monitoring/run',{method:'POST'});alert('Monitoramento: '+x.online+' online, '+x.errors+' com erro.');render()}catch(e){alert(e.message)}finally{rm.disabled=false}};
const pm=document.querySelector('#previewMerge');if(pm)pm.onclick=async()=>{const ids=[...document.querySelectorAll('.source-check:checked')].map(x=>x.value);if(!ids.length)return alert('Selecione ao menos uma fonte.');pm.disabled=true;try{const x=await api('/api/merge',{method:'POST',body:JSON.stringify({sourceIds:ids})});document.querySelector('#mergeResult').innerHTML='<section class="panel"><div class="toolbar"><div><h3>'+x.count+' itens únicos</h3><p class="muted">Prévia pronta para geração.</p></div><button class="primary" id="generate">Gerar fonte M3U</button></div><textarea class="preview" readonly>'+esc(x.preview)+'</textarea></section>';document.querySelector('#generate').onclick=async()=>{const name=prompt('Nome da fonte','LOS COLLECTOR - '+new Date().toLocaleDateString('pt-BR'));if(!name)return;const g=await api('/api/generated',{method:'POST',body:JSON.stringify({name,items:x.items})});alert('Fonte gerada.\n\nURL da playlist:\n'+g.publicUrl);S.page='generated';render()}}catch(e){alert(e.message)}finally{pm.disabled=false}};

...
function adminPasswordOk(password) {
  ...
  return Boolean(expected) && typeof password === 'string' && crypto.timingSafeEqual(
  ...
  ws.on('message', raw => {
    try {
      ...
      if (m.type === 'input' && role === 'controller') {
        safeSend(s.receiver, { type: 'input', control: m.control, payload: m.payload || {}, ts: m.ts || Date.now() });
      }
    } catch {}
  });
